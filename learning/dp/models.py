...
class ConditionalUnet1D(nn.Module):
    def __init__(
        ...
        """
        用嵌入向量而非标量来表示时间步t，因为对于变长序列，如果用标量表示的话，不好对时间步做归一化。diffusion_step_embed_dim是这个嵌入向量的维度。
        """
        ...
        diffusion_step_encoder = nn.Sequential(#时间步编码器
            SinusoidalPosEmb(dsed),# 正弦位置编码
            nn.Linear(dsed, dsed * 4),# 升维
            nn.Mish(),# 激活函数
            nn.Linear(dsed * 4, dsed),# 降维
        )
        cond_dim = dsed + global_cond_dim# 合并时间步编码和观测条件
