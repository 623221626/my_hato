...
class Agent:
    def __init__(
        ...
        if self.color_jitter:
            if self.image_channel == 3:
                # no depth
                self.downsample = nn.Sequential(#[原始RGB图像] → [调整尺寸到 240×320] → [随机亮度变化]
                    transforms.Resize(
                        (240, 320),
        ...
        """
        若 self.image_channel = 3（仅RGB）：
            mean_vec = [128.0, 128.0, 128.0]
            std_vec = [128.0, 128.0, 128.0]
        若 self.image_channel = 4（RGB+深度）：
            mean_vec = [128.0, 128.0, 128.0, 35767.0]
            std_vec = [128.0, 128.0, 128.0, 35767.0]
        """
        ...
        #[480×640 RGB图像] → [随机裁剪 216×288] → [标准化到 [-1,1]]
        self.transform = nn.Sequential(
            transforms.RandomCrop((216, 288)),#从原始图像中随机裁剪出 216×288 像素大小的区域,增加数据多样性
        ...
        if img_gaussian_noise > 0.0:#默认为0
            self.transform = nn.Sequential(
                self.transform,
        ...
        """
        [原始图像] → [中心裁剪 216×288] → [标准化到 [-1,1]]
        训练时使用 RandomCrop（随机位置裁剪）
        评估时使用 CenterCrop（固定位置裁剪）
        """
        ...
        self.policy = DiffusionPolicy(
            obs_horizon=obs_horizon,#模型在预测动作时考虑的历史观测帧数，默认为1
            obs_dim=obs_dim,# = 所有编码器输出特征的总维度 = eef_dim + hand_pos_dim + image_dim + pos_dim + touch_dim
            pred_horizon=pred_horizon,#模型一次预测的动作步数
            action_horizon=action_horizon,#每轮推理时实际执行的动作步数，影响在eval函数中控制每次从预测结果中取多少步动作
            action_dim=action_dim,
            representation_type=representation_type,
            encoders=encoders,# 各模态编码器
            num_diffusion_iters=num_diffusion_iters,
            without_sampling=without_sampling,#是否跳过扩散过程，直接使用简单行为克隆（BC）模型，默认为false
            weight_decay=weight_decay,#权重衰减率
            use_ddim=use_ddim,#是否使用确定性采样（DDIM）而非随机采样（DDPM）
            binarize_touch=self.binarize_touch,#是否将触觉传感器数据二值化（如>阈值=1，否则=0）
            policy_dropout_rate=policy_dropout_rate,
        )

        # Compile the forward function to accelerate deployment inference
        if self.compile_train:#默认为false
    ...
    def train(
        ...
        batch_size=4,
        epochs=300,
        traj_type="all",#指定训练数据的类型，例如"plain"
        prefix="",
        save_path=None,
        save_freq=10,#每隔多少个epoch保存一次模型
        eval_freq=10,#每隔多少个epoch运行一次评估
        wandb_logger=None,
        memmap_loader_path="",
        train_path=None,
        test_path=None,
        ...
        """
        加载训练集和测试集，
        然后用self.policy.train()训练模型
        """
        torch.cuda.empty_cache()

        if train_path is not None and test_path is not None:
            print(train_path)#'/home/zmj/my_ws/hato/split_data/data_banana_train_10'
            print(test_path)#'/home/zmj/my_ws/hato/split_data/data_banana_test'
            self.epi_dir = data_processing.get_epi_dir(#从 train_path 中筛选出所有符合 traj_type 和 prefix 的子文件夹
                train_path, traj_type=traj_type, prefix=prefix#traj_type='plain'
            )
            print(self.epi_dir)#'/home/zmj/my_ws/hato/split_data/data_banana_train_10/ 下的每个文件夹
            eval_trajs = data_processing.get_epi_dir(
                test_path, traj_type=traj_type, prefix=prefix#
            )
            assert len(eval_trajs) == 1
            eval_traj = eval_trajs[0]# 取第一个测试轨迹 例如'/home/zmj/my_ws/hato/split_data/data_banana_test/0307_225017'
            ...
            print("eval traj:", eval_traj)
        eval_data = self.get_eval_data(eval_traj)#貌似eval_data[0]是img,eef,hand_pos等数据，eval_data[1]是action

        train_loader = self.get_train_loader(batch_size, memmap_loader_path)#此时会把data_banana_train_10/下的那10条数据加载进来
        self.policy.set_lr_scheduler(len(train_loader) * epochs)#设置学习率调度器，动态调整模型的学习速度。使用 Cosine LR（先升温后下降）
        if self.stats is None:
            self.stats = train_loader.dataset.stats#eef, hand_pos等数据的最大最小值，用于归一化
            self.save_stats(save_path)
        if self.compile_train:
            train_loader.dataset.__getitem__ = torch.compile(#加速训练（实验性）
    ...
    def get_eval_data(self, data_path):
        ...
        data = data_processing.iterate(data_path)#data_path='/home/zmj/my_ws/hato/split_data/data_banana_test/0307_225017'

        action = self.get_eval_action(data)
        B = len(data)

        print("GETTING EVAL OBSERVATION", end="\r")
        obs = self.get_observation(data, load_img=True)#obs包含了img,eef,hand_pos等数据
        obs_list = []

        if "img" in self.representation_type:
            # transfer image type to float32
            obs["img"] = obs["img"].float()
            obs["img"] = self.eval_transform(obs["img"])#用神经网络处理
        for i in range(B):
    ...
    def eval(self, data_path, save_path=None):
        ...
        for i in range(len(action)):
            #print("Keys in eval_data[0][i]:", eval_data[0][i].keys())#,.mine
...
if __name__ == "__main__":
    ...
    args.add_argument("--train_suffix", type=str, default="")# data_banana_train_10中的 "_10"
    ...
    if args.load_path is not None:#默认为false
    ...
    if not args.eval:
        ...
        if args.use_train_test_split:# 默认true
            train_path = data_path + "_train" + args.train_suffix#train_suffix默认为空，可设定为 "_10", "_25" 等等
        ...
        #以上基本上都是在解析命令行参数###############################################################################
        agent.train(
            data_path,#'/home/zmj/my_ws/hato/split_data/data_banana'
            batch_size=args.batch_size,
            epochs=args.epochs,
            traj_type=args.traj_type,#plain。 traj_type="plain" 的作用是过滤掉不符合要求的轨迹目录，只保留常规（干净）数据
            prefix=args.prefix,#None
            save_path=model_path,#'/home/zmj/my_ws/hato/saved_models/0526_182525_hHn2-camera=012-identity=False-repr=IDEHTP-oh=1-ah=8-ph=16-prefix=None-do=0.0-imgos=32-wd=1e-05-use_ddim=False-binarize_touch=False'
            save_freq=args.save_freq,
            eval_freq=args.eval_freq,
            wandb_logger=wandb_logger,
            train_path=train_path,#'/home/zmj/my_ws/hato/split_data/data_banana_train_10'
            test_path=test_path,#'/home/zmj/my_ws/hato/split_data/data_banana_test'
